 ...
 * @Author: tgb
 * @Date: 2019-05-16 17:04:19
 * @LastEditors: tgb
 * @LastEditTime: 2019-05-16 17:11:08
 */
 ...
 * @param {Array} props
 */
function pickProps(source: any, props: any) {
  let target = {}
 ...
 * @returns wrapperCol
 */
function getLastFormItemLayout({ labelCol }: any) {
 ...
 */
function getLastIndex(dynamic = false, elements: any) {
  if (!dynamic) return elements.length - 1
  const els = elements.filter((v: any) => !!v)
  return els.length - 1
}

// 判断部件类型 渲染部件
function renderWidget(element: any, widgetProps: any) {
  let res
